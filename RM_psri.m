...
% -------------------------------------------------------------------------
% L2 Projections
% Precompute the projection coefficients for the basis and their gradients.
% -------------------------------------------------------------------------
p_phi_1 = zeros(nver,6);
p_phi_2 = zeros(nver,6);
g_phi_1 = zeros(nver,6);
g_phi_2 = zeros(nver,6);

for i=1:6
    %[p_phi_1(:,i), p_phi_2(:,i), g_phi_1(:,i), g_phi_2(:,i)]
    [tmp_p_1, tmp_p_2, tmp_g_1, tmp_g_2] = L2proj_phi(xv, yv, vertices, i);
%     disp('---------------------------');
%     disp(['Size p_phi_1: ' num2str(size(p_phi_1))]);
%     disp(['Size tmp_p_phi_1: ' num2str(size(tmp_p_1))]);
%     disp(['Size p_phi_2: ' num2str(size(p_phi_2))]);
%     disp(['Size tmp_p_phi_2: ' num2str(size(tmp_p_2))]);
%     disp(['Size g_phi_1: ' num2str(size(g_phi_1))]);
%     disp(['Size tmp_g_phi_1: ' num2str(size(tmp_g_1))]);
%     disp(['Size g_phi_2: ' num2str(size(g_phi_2))]);
%     disp(['Size tmp_g_phi_2: ' num2str(size(tmp_g_2))]);
    p_phi_1(:,i) = tmp_p_1;
    p_phi_2(:,i) = tmp_p_2;
    g_phi_1(:,i) = tmp_g_1;
    g_phi_2(:,i) = tmp_g_2;
    ...
    for i=1:12
        for j=1:12
            if j < 6.5  % First six basis functions (j)
                tmpJ = [p_phi_1([v1 v2 v3],j); ...
                        0; 0; 0];
             elseif j > 6.5 % Second six basis functions (i)
                tmpJ = [0; 0; 0; ...
                        p_phi_2([v1 v2 v3],j-6)];
            end
